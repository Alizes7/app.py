"""
NF-PDF → Excel Enterprise Ultra — app.py v2.2
Correções: Encoding, compatibilidade Python 3.14, código enxuto
"""
...
RE_DATA_HORA = re.compile(r"\b(\d{2}[\/\-\.]\d{2}[\/\-\.]\d{4})(?:[T\s,]+(\d{2}:\d{2}(?::\d{2})?))?", re.IGNORECASE)
RE_COMPETENCIA = re.compile(r"\b(\d{2}[\/\-\.]\d{4})\b")
RE_VALOR = re.compile(r"(?:R?\$[\s]*)?(\d{1,3}(?:\.\d{3})+,\d{2}|\d+,\d{2})(?!\d)", re.MULTILINE)
RE_VALOR_INTEIRO = re.compile(r"(?:R?\$[\s]*)?(\d{1,3}(?:\.\d{3})+|\d+)(?:,00)?(?=\s|$|[^\d,])", re.MULTILINE)

RE_NUMERO_NOTA = [
    re.compile(r"nfs?[- ]?e?\s*(?:n[°º#])?\s*[:\\-]?\s*(\d{9,})", re.IGNORECASE),
    re.compile(r"nf[- ]?e\s*(?:n[°º#])?\s*[:\\-]?\s*(\d{9,})", re.IGNORECASE),
    re.compile(r"nota\s*fiscal\s*(?:eletrônica\s*)?(?:n[°º#])?\s*[:\\-]?\s*(\d{9,})", re.IGNORECASE),
    re.compile(r"n[úu]mero\s+(?:da\s+)?nota\s*(?:fiscal\s*)?(?:n[°º#])?\s*[:\\-]?\s*(\d+)", re.IGNORECASE),
    re.compile(r"n[°º#]\s*da\s+nota\s*[:\\-]?\s*(\d{4,})", re.IGNORECASE),
    re.compile(r"rps\s*(?:n[°º#])?\s*[:\\-]?\s*(\d{6,})", re.IGNORECASE),
    re.compile(r"nota\s*(?:fiscal\s*)?(?:de\s*servi[çc]os?\s*)?(?:n[°º#]\s*|n[úu]mero\s*[:]?)s*(\d{4,}(?!\d{8}))", re.IGNORECASE),
    re.compile(r"(?:^|\s)N[°º#]\s*[:\\-]?\s*(\d{4,})(?!\d{6})", re.MULTILINE | re.IGNORECASE),
]
...
RE_ALIQUOTA = re.compile(r"al[íi]quota\s*(?:do\s*iss|iss)?\s*[:\\-]?\s*(\d+[,\.]\d+)\s*%?", re.IGNORECASE)
RE_ALIQUOTA_ALT = re.compile(r"(\d+[,\.]\d+)\s*%", re.IGNORECASE)

KEYWORD_MAP = {
    "numero_nota": ["número da nota", "nfs-e", "nfse", "nota fiscal", "n° da nota", "número nota", "no. nota", "n. nota", "número do rps", "rps", "nota de serviço", "numero", "nf-e", "número"],
    "data_emissao": ["data de emissão", "data emissão", "emissão", "data/hora", "emitida em", "data de competência", "competência", "data emitido", "data nota"],
    "competencia": ["competência", "mês competência", "período", "mês de referência", "referência"],
    "cnpj_prestador": ["cnpj prestador", "cnpj do prestador", "prestador cnpj", "cnpj/cpf prestador", "cpf/cnpj prestador", "emitente", "empresa prestadora", "prestador de serviços"],
    "razao_prestador": ["prestador de serviços", "prestador", "emitente", "empresa", "razão social prestador", "nome/razão social", "razão social do prestador"],
    "cnpj_tomador": ["cnpj tomador", "cnpj do tomador", "tomador cnpj", "cnpj/cpf tomador", "cpf/cnpj tomador", "tomador de serviços", "contratante"],
    "razao_tomador": ["tomador de serviços", "tomador", "contratante", "cliente", "razão social tomador", "nome/razão social do tomador"],
    "valor_servicos": ["valor total dos serviços", "valor bruto", "valor dos serviços", "total dos serviços", "valor total de serviços", "valor do serviço", "total bruto", "valor da nota", "valor total"],
    "valor_liquido": ["valor líquido", "valor a receber", "valor líquido da nota", "líquido da nfs", "valor líquido nfse", "total líquido", "vlr líquido"],
    "base_calculo": ["base de cálculo", "base de calc", "base calculo", "valor base", "base tributável"],
    "valor_iss": ["valor do iss", "iss calculado", "iss", "valor iss", "imposto sobre serviços", "valor do imposto"],
    "iss_retido": ["iss retido", "iss a reter", "retenção iss", "iss retido na fonte", "retido na fonte iss"],
    "pis": ["pis", "pis/pasep", "valor pis", "retenção pis"],
    "cofins": ["cofins", "valor cofins", "retenção cofins"],
    "csll": ["csll", "valor csll", "contribuição social"],
    "irrf": ["irrf", "ir", "irpj", "imposto de renda", "i.r.r.f", "retenção ir", "valor ir"],
    "inss": ["inss", "previdência", "contribuição previdenciária", "retenção inss"],
    "descricao_servico": ["discriminação dos serviços", "discriminação", "descrição", "histórico", "objeto", "serviços prestados", "atividade", "descrição do serviço"],
    "codigo_servico": ["código do serviço", "código de serviço", "código tributário", "código cnae", "cnae", "atividade econômica", "item da lista", "código atividade"],
    "municipio_prestacao": ["município de incidência", "local da prestação", "município de prestação", "município prestação", "local de prestação"],
    "desconto": ["descontos", "desconto incondicional", "desconto condicionado", "total de descontos"],
}
...
def extrair_melhor_valor(texto: str, contexto: Optional[str] = None) -> Optional[str]:
    if not texto:
        return None
    matches = RE_VALOR.findall(texto)
    if not matches:
        matches = RE_VALOR_INTEIRO.findall(texto)
    if not matches:
        return None
        ...
        try:
            ...
            for v in unicos:
                try:
                    f = limpar_valor(v)
                    if f is not None:
                        valores_float.append((f, v))
                except:
                    continue
            if valores_float:
                maior = max(valores_float, key=lambda x: x[0])
                return maior[1]
        except:
            pass
...
def texto_apos_keyword(texto_completo: str, keywords: List[str], janela: int = 120) -> Optional[str]:
...
def extrair_numero_nota(texto: str, debug: bool = False) -> Optional[str]:
    candidatos = []
    for idx, padrao in enumerate(RE_NUMERO_NOTA):
        matches = padrao.findall(texto)
        for m in matches:
            if isinstance(m, tuple):
                m = m[0]
            numero_limpo = re.sub(r"\D", "", m)
            if len(numero_limpo) == 14:
                continue
            if len(numero_limpo) == 11 and numero_limpo.startswith(('0', '1', '2', '3')):
                continue
            if 4 <= len(numero_limpo) <= 15:
                candidatos.append({'numero': numero_limpo, 'original': m.strip(), 'prioridade': idx, 'tamanho': len(numero_limpo)})
    
    if not candidatos:
        return None
    
    candidatos.sort(key=lambda x: (x['prioridade'], -x['tamanho']))
    
    if debug:
        logger.info(f"Candidatos a numero da nota: {candidatos[:3]}")
...
def extrair_valor_campo(texto: str, keywords: List[str], campo_nome: str = "", debug: bool = False) -> Optional[str]:
    trecho = texto_apos_keyword(texto, keywords, 80)
    if not trecho:
        return None
    valor = extrair_melhor_valor(trecho, contexto=campo_nome)
    if debug and valor:
        logger.info(f"[{campo_nome}] Trecho: '{trecho[:50]}...' -> Valor: {valor}")
    return valor
...
def extrair_iss_retido(texto: str) -> Optional[str]:
    keywords_sim = ["iss retido", "retido na fonte", "retenção iss", "iss a reter", "retido pelo tomador", "sim", "iss retido: sim"]
...
class ExtratorEstrategias:
    def __init__(self, texto_completo: str, blocos_posicionais: List[Dict], tabelas: List[List[List[str]]], modo_debug: bool = False):
        self.texto = texto_completo
        self.blocos = blocos_posicionais
    ...
    def _estrategia_regex(self) -> Dict[str, Any]:
        t = self.texto
        resultado = {}
        resultado["numero_nota"] = extrair_numero_nota(t, debug=self.modo_debug)
        ...
        resultado["valor_servicos"] = extrair_valor_campo(t, KEYWORD_MAP["valor_servicos"], "valor_servicos", self.modo_debug)
        resultado["valor_liquido"] = extrair_valor_campo(t, KEYWORD_MAP["valor_liquido"], "valor_liquido", self.modo_debug)
        resultado["base_calculo"] = extrair_valor_campo(t, KEYWORD_MAP["base_calculo"], "base_calculo", self.modo_debug)
        resultado["valor_iss"] = extrair_valor_campo(t, KEYWORD_MAP["valor_iss"], "valor_iss", self.modo_debug)
        resultado["pis"] = extrair_valor_campo(t, KEYWORD_MAP["pis"], "pis", self.modo_debug)
        resultado["cofins"] = extrair_valor_campo(t, KEYWORD_MAP["cofins"], "cofins", self.modo_debug)
        resultado["csll"] = extrair_valor_campo(t, KEYWORD_MAP["csll"], "csll", self.modo_debug)
        resultado["irrf"] = extrair_valor_campo(t, KEYWORD_MAP["irrf"], "irrf", self.modo_debug)
        resultado["inss"] = extrair_valor_campo(t, KEYWORD_MAP["inss"], "inss", self.modo_debug)
        resultado["desconto"] = extrair_valor_campo(t, KEYWORD_MAP["desconto"], "desconto", self.modo_debug)
    ...
    def _estrategia_keywords(self) -> Dict[str, Any]:
        ...
        for campo, kws in [("cnpj_prestador", KEYWORD_MAP["cnpj_prestador"]), ("cnpj_tomador", KEYWORD_MAP["cnpj_tomador"])]:
...
def processar_pdf(pdf_bytes: bytes, nome_arquivo: str, modo_debug: bool = False) -> List[Dict[str, Any]]:
    ...
    if modo_debug:
        logger.info(f"=== DEBUG: {nome_arquivo} ===")
        logger.info(f"Texto extraído (primeiros 500 chars): {texto_combinado[:500]}")
        logger.info(f"Blocos encontrados: {len(blocos)}")
        logger.info(f"Tabelas encontradas: {len(tabelas)}")
    
    extrator = ExtratorEstrategias(texto_combinado, blocos, tabelas, modo_debug=modo_debug)
...
def gerar_excel(df: pd.DataFrame) -> bytes:
    ...
    dados_stat = [
        ...
        ("Sistema", "NF-PDF → Excel Enterprise Ultra v2.2"),
    ]
...
def renderizar_hero() -> None:
    ...
    <div class="hero-banner">
        ...
        <span class="hero-badge">⚡ Hybrid Engine · pdfplumber + PyMuPDF · 4-Layer Cascade · v2.2</span>
    </div>
...
def renderizar_sidebar() -> Dict[str, Any]:
    with st.sidebar:
        ...
        opcoes = {
            ...
            "modo_debug": st.checkbox("🔍 Modo Debug (logs detalhados)", value=False,
                                       help="Ative para ver o que está sendo encontrado nos PDFs"),
...
def renderizar_metricas(total: int, sucesso: int, alertas: int, tempo: float) -> None:
    ...
    if st.button("▶️ Processar Notas Fiscais", type="primary", use_container_width=True):
        ...
        for i, arquivo in enumerate(uploaded):
            ...
            try:
                ...
                linhas = processar_pdf(pdf_bytes, nome, modo_debug=opcoes["modo_debug"])
