"""
╔══════════════════════════════════════════════════════════════════════╗
║         NF-PDF → Excel Enterprise Ultra — app.py (V2.1)             ║
║  Correções: Extração de Número da Nota e Valores Monetários         ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# SEÇÃO 1: CONSTANTES E PADRÕES DE REGEX (MELHORADOS)
# ══════════════════════════════════════════════════════════════════════════════
...
# MELHORIA: Padrão de valor monetário brasileiro mais robusto
# Evita capturar CNPJ (14 dígitos) ou telefones
RE_VALOR = re.compile(
    r"(?:R?\$[\s]*)?(\d{1,3}(?:\.\d{3})+,\d{2}|\d+,\d{2})(?!\d)",  # 1.234,56 ou 123,45 (não seguido de dígito)
    re.MULTILINE
)

# Padrão alternativo para valores sem centavos
RE_VALOR_INTEIRO = re.compile(
    r"(?:R?\$[\s]*)?(\d{1,3}(?:\.\d{3})+|\d+)(?:,00)?(?=\s|$|[^\d,])",
    re.MULTILINE
)

# MELHORIA: Padrões de número da nota mais específicos
# Ordem de prioridade: do mais específico para o mais genérico
RE_NUMERO_NOTA = [
    # NFS-e / NF-e específico com numeração grande (evita CNPJ)
    ...
    # Número em contexto de nota (evita CNPJ)
    re.compile(r"nota\s*(?:fiscal\s*)?(?:de\s*servi[çc]os?\s*)?(?:n[°º#]\s*|n[úu]mero\s*[:]?)\s*(\d{4,}(?!\d{8}))", re.IGNORECASE),
    
    # Padrão genérico só como último recurso (evita CNPJ/CPF)
...
KEYWORD_MAP = {
    ...
    "valor_servicos": [
        ...
        "total bruto", "valor da nota", "valor total",
    ],
    "valor_liquido": [
...
# MELHORIA: Função mais robusta para extrair valores
def extrair_melhor_valor(texto: str, contexto: Optional[str] = None) -> Optional[str]:
    ...
    Args:
        texto: Texto onde procurar o valor
        contexto: Contexto adicional para debug
    """
    if not texto:
        return None
    
    # Primeiro tenta o padrão com centavos (mais específico)
    matches = RE_VALOR.findall(texto)
    
    if not matches:
        # Tenta padrão de valor inteiro
    ...
    # Se houver múltiplos valores, pega o maior (geralmente é o valor total)
    # ou o primeiro se só tiver um
    if len(unicos) > 1:
        try:
            ...
            if valores_float:
                # Retorna o maior valor encontrado (geralmente o total)
...
# ══════════════════════════════════════════════════════════════════════════════
# SEÇÃO 3: EXTRATORES DE CAMPOS ESPECÍFICOS (MELHORADOS)
# ══════════════════════════════════════════════════════════════════════════════
...
# MELHORIA: Função melhorada para extrair número da nota
def extrair_numero_nota(texto: str, debug: bool = False) -> Optional[str]:
    """
    Extrai número da NFS-e / NF-e / RPS com validações anti-CNPJ.
    
    Estratégia:
    1. Procura padrões específicos de nota fiscal (evita CNPJ)
    2. Valida que não é um CNPJ (14 dígitos) ou CPF (11 dígitos)
    3. Prioriza números grandes (9+ dígitos) típicos de NF-e
    """
    candidatos = []
    ...
    for idx, padrao in enumerate(RE_NUMERO_NOTA):
        ...
        for m in matches:
            if isinstance(m, tuple):
                m = m[0]  # Se o grupo de captura for uma tupla
            
            numero_limpo = re.sub(r"\D", "", m)
            
            # VALIDAÇÃO: Não pode ser CNPJ (14 dígitos) ou CPF (11 dígitos)
            if len(numero_limpo) == 14:
                continue  # Provavelmente é CNPJ
            if len(numero_limpo) == 11 and numero_limpo.startswith(('0', '1', '2', '3')):
                # Pode ser CPF, mas vamos verificar se passa na validação de CPF
                continue
            
            # VALIDAÇÃO: Deve ter entre 4 e 15 dígitos (padrão de notas fiscais)
    ...
    # Ordena por: prioridade do padrão (menor índice = melhor), depois por tamanho (maior = melhor)
    candidatos.sort(key=lambda x: (x['prioridade'], -x['tamanho']))
    
    if debug:
        logger.info(f"Candidatos a número da nota: {candidatos[:3]}")
...
# MELHORIA: Função de extração de valor com debug
def extrair_valor_campo(texto: str, keywords: List[str], campo_nome: str = "", debug: bool = False) -> Optional[str]:
    ...
    Args:
        ...
        campo_nome: Nome do campo para debug
        debug: Se True, loga informações de debug
    """
    trecho = texto_apos_keyword(texto, keywords, 80)  # Aumentei a janela para 80
    if not trecho:
        return None
...
# ══════════════════════════════════════════════════════════════════════════════
# SEÇÃO 4: ESTRATÉGIAS DE EXTRAÇÃO EM CASCATA (ATUALIZADA)
# ══════════════════════════════════════════════════════════════════════════════
...
class ExtratorEstrategias:
    ...
    def __init__(self, texto_completo: str, blocos_posicionais: List[Dict],
                 tabelas: List[List[List[str]]], modo_debug: bool = False):
...
def processar_pdf(pdf_bytes: bytes, nome_arquivo: str, modo_debug: bool = False) -> List[Dict[str, Any]]:
    ...
    # Extração em cascata (com modo debug)
...
# ══════════════════════════════════════════════════════════════════════════════
# SEÇÃO 6: GERAÇÃO DO EXCEL PROFISSIONAL (mantida igual)
# ══════════════════════════════════════════════════════════════════════════════
...
def gerar_excel(df: pd.DataFrame) -> bytes:
    ...
    dados_stat = [
        ...
        ("Sistema", "NF-PDF → Excel Enterprise Ultra v2.1"),
    ]
...
# ══════════════════════════════════════════════════════════════════════════════
# SEÇÃO 7: INTERFACE STREAMLIT — COM MODO DEBUG
# ══════════════════════════════════════════════════════════════════════════════
...
def renderizar_hero() -> None:
    ...
    <div class="hero-banner">
        ...
        <span class="hero-badge">⚡ Hybrid Engine · pdfplumber + PyMuPDF · 4-Layer Cascade · v2.1</span>
    </div>
    ...
    if st.button("▶️ Processar Notas Fiscais", type="primary", use_container_width=True):
        ...
        for i, arquivo in enumerate(uploaded):
            ...
            try:
                ...
                # Passa o modo debug para o processamento
